...
# Colocando Week como uma variável inteira, pois ela está como Carácter.
...
plot(
  Sepal.Width ~ fct_reorder(Species, Sepal.Width,
...
iris$Species <- fct_recode(Species, Sp1 = "setosa", Sp2 = "versicolor", Sp3 = "virginica")
...
glimpse(peixes)
peixes_1 <- peixes |> mutate(across(everything(), ~ replace_na(.x, 0)))
...
grafico_1 + geom_boxplot(aes(fill = Species)) +
  scale_fill_manual(values = c("#D73027", "#FEE090", "#91BFDB"))
...
f + geom_jitter(aes(shape = Species, size = 5))

# d. Histograma
g <- ggplot(iris, aes(Sepal.Length))
g + geom_histogram(
  color = "black",
  bins = 10 ,# N° de Barras, padrão = 30
)
# e. Densidade
g + geom_density()
